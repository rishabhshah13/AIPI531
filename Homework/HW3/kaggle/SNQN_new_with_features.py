...
def parse_args():
    ...
    parser.add_argument('--item_features_loc', default='', type=str)
    parser.add_argument('--lamda_value', default=0.1, type=float)
...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain,features,name='DQNetwork',lambda_value=0.1):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.phi_s = tf.matmul(self.states_hidden, self.features_layer, transpose_b=True)

            self.fs = ( 1 - self.lamda ) * self.output1 + self.lamda * self.phi_s
            ...
            ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.fs)
...
def evaluate(sess):
    ...
    while evaluated<len(eval_ids):
        ...
        prediction=sess.run(QN_1.fs, feed_dict={QN_1.inputs: states,QN_1.len_state:len_states,QN_1.is_training:False})
...
if __name__ == '__main__':
    ...
    item_features_loc = args.item_features_loc
    reward_click = args.r_click
    lambda_value=args.lamda_value
    ...
    #Reading item features from one-hot encoded file
    item_features = pd.read_csv(os.path.join(data_directory, item_features_loc)).values.astype('float32')
    
    QN_1 = QNetwork(name='QN_1', hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
                    state_size=state_size, pretrain=False,features=item_features,lambda_value=lambda_value)
